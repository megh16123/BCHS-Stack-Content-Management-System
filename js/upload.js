...
const file_extension_validato =(file_name)=>{
	...
	const ban = ["doc","docm","docx","docx","dot","dotm","dotx","mht","mhtml","odt","rtf","wps","xps","pptx"]
	console.log(file)
	if(file.length>2){
...
const file = document.getElementById("myinput");
	...
	if(file_size_validator(file.files[0])){
		...
		const method = "POST";
		const payload = sessionStorage.getItem("token")+file.files[0].name+event.target.result.split(",")[1];
